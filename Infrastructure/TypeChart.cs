namespace TeamAceProject.Infrastructure
{
    public static class TypeChart
    {
        ...
        // Types that deal >1x total damage against a Pokemon with the given defending types.
        // Returns (typeName, multiplier) pairs grouped as 4x then 2x.
        public static IEnumerable<(string Type, double Multiplier)> GetWeaknesses(IEnumerable<string> defenderTypes)
        {
            var defIndexes = defenderTypes
                .Where(t => TypeIndex.ContainsKey(t))
                .Select(t => TypeIndex[t])
                .ToList();

            return Types
                .Select((atk, atkIdx) =>
                {
                    double mult = defIndexes.Aggregate(1.0, (acc, defIdx) => acc * Chart[atkIdx][defIdx]);
                    return (Type: atk, Multiplier: mult);
                })
                .Where(x => x.Multiplier > 1.0)
                .OrderByDescending(x => x.Multiplier);
        }

        // Types that a Pokemon with the given attacking types deals >1x damage to.
        // Returns (typeName, multiplier) pairs grouped as 4x then 2x.
        public static IEnumerable<(string Type, double Multiplier)> GetSuperEffectiveAgainst(IEnumerable<string> attackerTypes)
        {
            var atkIndexes = attackerTypes
                .Where(t => TypeIndex.ContainsKey(t))
                .Select(t => TypeIndex[t])
                .ToList();
            ...
            return Types
                .Select((def, defIdx) =>
                {
                    double mult = atkIndexes.Max(atkIdx => Chart[atkIdx][defIdx]);
                    return (Type: def, Multiplier: mult);
                })
                .Where(x => x.Multiplier > 1.0)
                .OrderByDescending(x => x.Multiplier);
        }
